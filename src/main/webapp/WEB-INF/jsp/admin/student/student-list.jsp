...
<div class="page-container">
    <div class="text-c"> 日期范围：
        ...
        <input type="text" class="input-text" style="width:250px" placeholder="<c:if test="${text != null}">${text}</c:if><c:if test="${text == null}">输入学生名称、电话、邮箱、学号</c:if>" id="selectByType" name="selectByType">
        <button type="submit" class="btn btn-success radius" id="selectButton" name="selectButton"><i class="Hui-iconfont">&#xe665;</i> 搜用户</button>
    </div>
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <tbody>
            <c:if test="${studentByTypes == null}">
                <c:forEach items="${students}" var="student">
                    <tr class="text-c">
                        <td><input type="checkbox" value="1" name=""></td>
                        <td>${student.id}</td>
                        <td><u style="cursor:pointer" class="text-primary" onclick="member_show('${student.contact_name}的个人信息','${pageContext.request.contextPath }/admin/student-show.action','${student.id}','360','400')">${student.contact_name}</u></td>
                        <td>${student.contact_subject}</td>
                        ...
                        <td class="td-manage"><a style="text-decoration:none" onClick="member_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a> <a title="编辑" href="javascript:;" onclick="member_edit('${student.contact_name}的编辑','${pageContext.request.contextPath }/admin/student-updateUI.action','${student.id}','470','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="member_del(this,'${student.id}','${pageContext.request.contextPath }/admin/student-delect.action')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
                    </tr>
                </c:forEach>
            </c:if>
            <c:if test="${studentByTypes != null}">
                <c:forEach items="${studentByTypes}" var="student">
                    <tr class="text-c">
                        <td><input type="checkbox" value="1" name=""></td>
                        <td>${student.id}</td>
                        <td><u style="cursor:pointer" class="text-primary" onclick="member_show('${student.contact_name}的个人信息','${pageContext.request.contextPath }/admin/student-show.action','${student.id}','360','400')">${student.contact_name}</u></td>
...
<script type="text/javascript">
    ...
    $(function (){
        ...
        $("#selectButton").click(function () {
            var text = $("#selectByType").val();
            alert(text);
            if(text != "") {
                $.ajax({
                    type: 'POST',
                    url: '${pageContext.request.contextPath }/admin/student-selectByType.action',
                    dataType: 'json',
                    data: {text : text},
                    success: function(data){
                        if(data.result){
                            ...
                            layer.msg('搜索错误，用户不存在,请重新搜索',{icon:1,time:1000});
                        }
                    },
                });
            }
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.result){
                        ...
                        layer.msg('删除错误，用户不存在',{icon:1,time:1000});
                    }
                },
                error:function(data) {
